...
function DialogNuevaClinica({
  ...
  function reiniciar() {
    ...
    setPassword(generarPasswordSugerida())
  }
  ...
  function guardar() {
    startTransition(async () => {
      const r = await crearClinica({
        ...
        plan_id: planId, telefono, direccion, password_admin: password,
      })
      if (!r.ok) { toast.error(r.error ?? "No se pudo crear la clínica."); return }
      if (r.error) toast.warning(r.error)
      if (r.password) {
        setEmailCreado(emailAdmin)
        setPasswordCreada(r.password)
      } else {
  ...
  return (
    <>
      <Dialog open={abierta} onOpenChange={(o) => { if (!o) onCerrar() }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Se crea la cuenta, la clínica, su suscripción y el acceso de la administradora con
              la contraseña que definas abajo — cópiala y entrégala por un canal seguro.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="password-nueva-clinica">Contraseña temporal de la administradora</Label>
              <div className="flex items-center gap-2">
                <Input
                  id="password-nueva-clinica"
                  value={password}
              ...
              <p className="text-[11px] text-muted-foreground">
                Se generó una automáticamente; puedes editarla si prefieres poner la tuya.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={guardar}
              disabled={isPending || !nombre.trim() || !emailAdmin.trim() || !planId || password.trim().length < 8}
            >
      ...
      <PasswordRevealDialog
        ...
        onClose={() => setPasswordCreada(null)}
        email={emailCreado || undefined}
        titulo="Clínica creada — contraseña de la administradora"
      />
    </>
  )
}
